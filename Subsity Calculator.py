# -*- coding: utf-8 -*-
"""
Created on Mon Aug 10:18:08 2019
...
NORMAL_FONT = ("Garmond", 14)
LARGE_FONT = ("Garmond", 16)
SMALL_FONT = ("Garmond", 8)


class PickleData:
    def picklesave(filename, filetype, varname):
        picklesave = open(filename, filetype)
        pickle.dump(varname, picklesave)
        picklesave.close()

    def pickleload(filename, filetype):
        pickleload = open(filename, filetype)
...
class Membership(tk.Frame):
    ...
    def duplicate(dupvalue):
        SCData = PickleData.pickleload("SCData.pickle", "rb")
        i = -1
        for n in SCData['bynamelist']:
            i = i+1
            if SCData['bynamelist'][i] == dupvalue:
                return True
            else:
                return False

    def combofillunit(self):
        SCData = PickleData.pickleload("SCData.pickle", "rb")
        byunitlist = {}
        for n in members:
            if self.duplicate(self, members, 'UNIT', members[n]['UNIT']):
                continue
            else:
                byunitlist.extend(members[n]['UNIT'])
        return byunitlist

    def combofillname(self):
        SCData = PickleData.pickleload("SCData.pickle", "rb")
        for n in SCData['members']:
            if (Membership.duplicate(SCData['members'][n]['NAME'])):
                continue
            else:
                SCData['bynamelist'].append(SCData['members'][n]['NAME'])
                SCData = {
                        "members": SCData['members'],
                        "ratescale": SCData['ratescale'],
                        "byunitlist": SCData['byunitlist'],
                        "bynamelist": SCData['bynamelist']
                        }
                picklesave = open("SCData.pickle", "wb")
                pickle.dump(SCData, picklesave)
                picklesave.close()
        return SCData['bynamelist']

    def selectbyname(name):
        SCData = PickleData.pickleload("SCData.pickle", "rb")
        for n in SCData['members']:
            if SCData['members'][n]['NAME'] == name:
                return SCData['members'][n]['unit']

        tk.messagebox.showinfo("INPUT ERROR", "Name is not valid")
        return

    def checkkey(self, selectunit, selectname):
        if (len(selectunit) > 0):
            self.key = selectunit
        elif (len(selectname) > 0):
            self.key = selectname(selectunit)
        else:
            tk.messagebox.showinfo("INPUT ERROR", "You must choose a Unit or Name")
        winsound.MessageBeep(1)


class RentScales(tk.Frame):
    def __init__(self, parent, controller):
        tk.Frame.__init__(self, parent)


class SCInput(tk.Frame):
    def __init__(self, parent, controller):
#        tk.Tk.title(self, "Subsidy Calculor Input")
        SCData = PickleData.pickleload("SCData.pickle", "rb")
        tk.Frame.__init__(self, parent)
        SCInputFrm = tk.Frame(self)
        SCInputFrm.grid(row=1, column=0, sticky="nsew")
        SCInputFrm.grid_rowconfigure(0, weight=1)
        SCInputFrm.grid_columnconfigure(0, weight=1)
        SCInputFrm.selectcombo = self.selectcombo()
        SCInputFrm.lblselect = ttk.Label(self, text='Select Unit', font=SMALL_FONT)
        SCInputFrm.lblselect.grid(row=0, column=1)
        SCInputFrm.selectunit = tk.StringVar()
        SCInputFrm.selectbyunit = ttk.Combobox(self, width=4, textvariable=SCInputFrm.selectunit, font=SMALL_FONT)
        SCInputFrm.selectbyunit['values'] = SCData['byunitlist']
        SCInputFrm.selectbyunit.grid(row=0, column=2)
        SCInputFrm.lblor = ttk.Label(self, text='  or Name ', font=SMALL_FONT)
        SCInputFrm.lblor.grid(row=0, column=3)
        SCInputFrm.selectname = tk.StringVar()
        SCInputFrm.selectbyname = ttk.Combobox(self, width=20, textvariable=SCInputFrm.selectname, font=SMALL_FONT)
        SCInputFrm.selectbyname['values'] = Membership.combofillname(self)
        SCInputFrm.selectbyname.grid(row=0, column=4)
        SCInputFrm.selectbutton = ttk.Button(self,text='Select',  command=Membership.checkkey(self,SCInputFrm.selectbyunit.get(), SCInputFrm.selectbyname.get()))
        SCInputFrm.selectbutton.grid(row=0, column=5)
        SCInputFrm.lblselectstartdate = ttk.Label(self, text='Start Date', font=SMALL_FONT)
        SCInputFrm.lblselectstartdate.grid(row=0, column=6)
        today = datetime.date.today()
        SCInputFrm.selectstartdate = DateEntry(self, width=12, background='black',
foreground='white', borderwidth=2, year=today.year)
        SCInputFrm.selectstartdate.grid(row=0,column=7)
        SCInputFrm.lblselectprintdate= ttk.Label(self,text='Print Date', font=SMALL_FONT)
        self.data = {}
        SCInputFrm.data = {}
        SCInputFrm.lblselectprintdate.grid(row=0, column=9)
        today = datetime.date.today()
        SCInputFrm.printdate = DateEntry(self, width=12, background='black',
                    foreground='white', borderwidth=2, year=today.year)
        SCInputFrm.printdate.grid(row=0, column=10)
        SCInputFrm.lblschedulea=  ttk.Label(self, text = 'Schedule A', font=SMALL_FONT)
        SCInputFrm.lblschedulea.grid(row=1, column=5)
        SCInputFrm.lblsubsidyagreement =  ttk.Label(self, text = 'Subsidy Agreement', font=SMALL_FONT)
        SCInputFrm.lblsubsidyagreement.grid(row=2, column=5)
        SCInputFrm.lblfigmaychg =  ttk.Label(self, text = 'These figures may change from time to time', font=SMALL_FONT)
        SCInputFrm.lblfigmaychg.grid(row=3, column=1)

    def popup(self, frame, date):
        cal = Calendar(frame, date)
...
#        print(self.data)
    ...
    def selectcombo(self):
        ttk.Label(text='you have selected')


class SCOutput(tk.Frame):
    def __init__(self, parent, controller):
        SCData = PickleData.pickleload("SCData.pickle", "rb")
        tk.Frame.__init__(self, parent)
        SCOutputFrm = tk.Frame(self)
        SCOutputFrm.grid(row=1, column=0, sticky="nsew")
        SCOutputFrm.grid_rowconfigure(0, weight=1)
        SCOutputFrm.grid_columnconfigure(0, weight=1)
#        SCOutputFrm.header =  (file="Shamrock Letterhead.jpg")
        SCOutputFrm.lblschedulea = ttk.Label(self, text='Schedule A', font=SMALL_FONT)
        SCOutputFrm.lblschedulea.grid(row=1, column=5)
        SCOutputFrm.lblsubsidyagreement =  ttk.Label(self, text='Subsidy Agreement', font=SMALL_FONT)
        SCOutputFrm.lblsubsidyagreement.grid(row=2, column=5)
        SCOutputFrm.lblfigmaychg =  ttk.Label(self, text='These figures may change from time to time', font=SMALL_FONT)
        SCOutputFrm.lblfigmaychg.grid(row=3, column=1)


class SCPrint(tk.Frame):
...
class HelpOverview(tk.Frame):
    pass
#    def __init__(self, parent, controller):
#        tk.Frame.__init__(self, parent)


class HelpInput(tk.Frame):
    pass
#    def __init__(self, parent, controller):
#        tk.Frame.__init__(self, parent)


class HelpOutput(tk.Frame):
    pass
#    def __init__(self, parent, controller):
#        tk.Frame.__init__(self, parent)


class HelpCode(tk.Frame):
    pass
#    def __init__(self, parent, controller):
#        tk.Frame.__init__(self, parent)
...
class SubsityCalculator(tk.Tk):
    def __init__(self, *args, **kwargs):
        tk.Tk.__init__(self, *args, **kwargs)
        tk.Tk.title(self, "Subsidy Calculor")
        tk.Tk.iconbitmap(self, default="SCIcon.ico")
        ...
        menubar.add_command(label="Input", command=lambda: self.show_frame(SCInput))
        menubar.add_command(label="Output", command=lambda: self.show_frame(SCOutput))
        menubar.add_command(label="Print", command=lambda: self.show_frame(SCPrint))
        settingmenu = tk.Menu(menubar, tearoff=0)
        settingmenu.add_command(label="Members",command=lambda: self.show_frame(Membership))
        settingmenu.add_command(label="Units", command=lambda: self.show_frame(Units))
        settingmenu.add_command(label="RentScale", command=lambda: self.show_frame(RentScales))
        settingmenu.add_command(label="Codes", command=lambda: self.show_frame(Codes))
        settingmenu.add_command(label="Calendar", command=lambda: self.show_frame(Calendar))
        menubar.add_cascade(label="Setting", menu=settingmenu)
        helpmenu = tk.Menu(menubar, tearoff=0)
        helpmenu.add_command(label="Overview", command=lambda: self.show_frame(HelpOverview))
        helpmenu.add_command(label="Input", command=lambda: self.show_frame(HelpInput))
        helpmenu.add_command(label="Output", command=lambda: self.show_frame(HelpOutput))
        helpmenu.add_command(label="Codes", command=lambda: self.show_frame(HelpCode))
...
#                  HelpCode
#                  }:
            frame = F(self.parent, self)
            self.frames[F] = frame
            frame.grid(row=1, column=0, sticky="nsew")

        self.show_frame(SCInput)
